...
const WriteForm = () => {
  ...
  const onSaveWrite = () => {
    ...
    axios
      .post("http://localhost:8080/api/devlog/save", writeDTO, {
        headers: {
  ...
  useEffect(() => {
    // 카테고리 리스트 불러오기
    axios.get("http://localhost:8080/api/devlog/getCategoryList").then((res) => {
      setCategoryList(res.data);
    });
    // 태그 리스트 불러오기
    axios.get("http://localhost:8080/api/devlog/getTagList").then((res) => {
      setTagList(res.data);
    });
  }, []);
