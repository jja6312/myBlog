package com.example.demo.studyTime.repository;
...
import com.example.demo.studyTime.bean.StudyTime;
import com.example.demo.studyTime.bean.StudyTimeByDayGroupByCategoryDTO;
import com.example.demo.studyTime.bean.StudyTimeSummaryDTO;
...
public interface StudyTimeRepository extends JpaRepository<StudyTime, Long> {
	...
	@Query("SELECT new com.example.demo.studyTime.bean.StudyTimeSummaryDTO(FUNCTION('DATE', st.startTime) as date, SUM(st.durationInSeconds)) "
			+
			"FROM StudyTime st WHERE st.startTime BETWEEN :startDate AND :endDate " +
			"GROUP BY FUNCTION('DATE', st.startTime)")
	...
	//클릭된 날짜에 대한 공부종류
	@Query("SELECT new com.example.demo.studyTime.bean.StudyTimeByDayGroupByCategoryDTO(SUM(st.durationInSeconds) / 60, c.name) " +
			"FROM StudyTime st " +
			"JOIN st.category c " +
