...
public class StudyTimeServiceImpl implements StudyTimeService {
    ...
    public StudyTimeAverageDTO getAverageStudyTime() {
        ...
        Double avgWeekdays = studyTimeRepository.findAverageWeekdayStudyTimeInMinutes();//평일
        if(avgWeekdays==null) avgWeekdays= (double) 0;
        avgWeekdays = Math.floor(avgWeekdays*100)/100;

        Double avgWeekends = studyTimeRepository.findAverageWeekendStudyTimeInMinutes();//주말
        if(avgWeekends==null) avgWeekends= (double) 0;
        avgWeekends = Math.floor(avgWeekends*100)/100;




        StudyTimeAverageDTO studyTimeAverageDTO = new StudyTimeAverageDTO(avgWeekdays,avgWeekends,0);
