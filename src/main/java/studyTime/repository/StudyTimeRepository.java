...
public interface StudyTimeRepository extends JpaRepository<StudyTime, Long> { 
	...
	@Query(value = "SELECT SUM(duration_in_seconds) / 60 / 5 AS dayStudyTimeAsMinute " +
			"FROM study_time " +
	...
	@Query(value = "SELECT SUM(duration_in_seconds) / 2 / 60 AS weekendStudyTimeAsMinute " +
			"FROM study_time " +
...
//	List<StudyTime> findByStartTimeBetween(LocalDateTime startDateTime, LocalDateTime endDateTime);
//
//	// 주말에 해당하는 StudyTime 데이터만 조회
//	@Query("SELECT st FROM StudyTime st WHERE st.startTime >= :startDateTime AND st.endTime <= :endDateTime AND FUNCTION('DAYOFWEEK', st.startTime) IN (1, 7)")
//	List<StudyTime> findWeekendsBetweenDates(@Param("startDateTime") LocalDateTime startDateTime, @Param("endDateTime") LocalDateTime endDateTime);
